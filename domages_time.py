...
for i in range(init_Iter-1,Iter):
	...
	#solver.parameters["cg_solver"]["monitor_convergence"] = True
	solver.parameters["krylov_solver"]["monitor_convergence"] = True
	solver.parameters["linear_solver"]="cg"
	solver.parameters["preconditioner"]="none"
	print("linear solver",solver.parameters["linear_solver"])
	print("relative tolerance",solver.parameters["linear_solver"]["relative_tolerance"])
	#print(info(solver.default_parameters(), True))
	solver.solve()
...
if Output_latex == True:
	...
	f.write('delta : \n')	
	output_latex(f,delta_vec,delta_vec)
	f.close()
